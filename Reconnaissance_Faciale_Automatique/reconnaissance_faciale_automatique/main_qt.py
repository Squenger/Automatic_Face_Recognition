...
class FaceRecoApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # --- SECTION 2bis : GESTION DE LA BASE ---
        mgmt_group = QGroupBox("Gestion de la Base")
        mgmt_layout = QHBoxLayout()

        self.btn_delete_person = QPushButton("Supprimer une personne")
        self.btn_delete_person.clicked.connect(self.delete_person_ui)
        self.btn_delete_person.setStyleSheet("background-color: #e67e22; color: white;")

        self.btn_clear_db = QPushButton("Vider la base")
        self.btn_clear_db.clicked.connect(self.clear_database_ui)
        self.btn_clear_db.setStyleSheet("background-color: #e74c3c; color: white;")

        mgmt_layout.addWidget(self.btn_delete_person)
        mgmt_layout.addWidget(self.btn_clear_db)
    ...
    def delete_person_ui(self):
        """Affiche une boîte de dialogue pour supprimer une personne par son nom."""
        success, count = self.manager.load_encodings()
        if not success or count == 0:
            QMessageBox.warning(self, "Erreur", "La base de données est vide.")
            return
